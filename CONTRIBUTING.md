...
- **Planned improvements** — see [`backlog/`](backlog/) for a
...
The [`backlog/`](backlog/) folder holds the current, concrete backlog:
`backlog/PLAN-MEJORA-CODE-RAG-MANAGER.md` is the design document behind
it (rationale, theory, and every decision already made), and
`backlog/README.md` indexes the individual `US-NN-*.md` user stories
...
is deleted from `backlog/` in the same PR (or a prompt follow-up), and the
entry is removed from `backlog/README.md`. If you're looking for
